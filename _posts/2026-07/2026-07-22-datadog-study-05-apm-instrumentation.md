...
3. **Context Propagation(컨텍스트 전파)** — 서비스 A가 서비스 B를 호출할 때, trace_id와 현재 span ID를 HTTP 헤더나 gRPC 메타데이터에 실어서 같이 보냄. 데이터독은 [자체 헤더](https://docs.datadoghq.com/tracing/trace_collection/trace_context_propagation/)(`x-datadog-trace-id`, `x-datadog-parent-id` 등)를 기본으로 쓰면서 W3C 표준 `traceparent`도 함께 지원한다(기본 추출 순서: datadog → tracecontext → baggage) — OpenTelemetry 표준만 쓰는 서비스와도 섞어 쓸 수 있는 이유
...
6. **[Unified Service Tagging](https://docs.datadoghq.com/getting_started/tagging/unified_service_tagging/)** — 각 서비스가 자기소개를 하는 방식. `DD_ENV`/`DD_SERVICE`/`DD_VERSION` 환경변수를 표준화해서 지정하면 트레이스·메트릭·로그 전부에 동일한 env/service/version 태그가 자동으로 붙어서 일관되게 필터링할 수 있음
...
flowchart TB
    ...
    F1 -->|"x-datadog-trace-id=abc123<br/>(traceparent도 지원)"| G1
