...
- [컨트롤러](https://kubernetes.io/docs/concepts/architecture/controller/)가 관찰(Watch) → 조정(Reconcile) 루프를 끊임없이 돌면서 선언된 상태와 실제 상태를 맞춘다
...
3. **조정([Reconcile](https://kubernetes.io/docs/concepts/architecture/controller/#controller-pattern)):** 선언과 실제 상태가 다르면 자동으로 맞춤 ("1개 더 띄워야겠다")
4. **반복:** 이 관찰-조정 루프가 끊임없이 돌면서, 컨테이너가 죽으면 즉시 다시 띄우고, 서버가 다운되면 다른 서버로 옮김

이 "선언하면 알아서 유지해준다"는 방식을 [선언적(declarative) 방식](https://kubernetes.io/docs/concepts/overview/working-with-objects/)이라 부르고, K8s의 핵심 철학이다.
